...
import { TokenAmount, Token, ChainId, Percent, JSBI } from '@complus/sdk-ava'
...
describe('utils', () => {
  describe('#getCChainLink', () => {
    it('correct for tx', () => {
      expect(getCChainLink(1, 'abc', 'transaction')).toEqual('https://snowtrace.io/tx/abc')
    })
    it('correct for token', () => {
      expect(getCChainLink(1, 'abc', 'token')).toEqual('https://snowtrace.io/token/abc')
    })
    it('correct for address', () => {
      expect(getCChainLink(1, 'abc', 'address')).toEqual('https://snowtrace.io/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getCChainLink(2, 'abc', 'address')).toEqual('https://snowtrace.io/address/abc')
    })
    it('testnet', () => {
      expect(getCChainLink(3, 'abc', 'address')).toEqual('https://testnet.snowtrace.io/address/abc')
    })
    it('enum', () => {
      expect(getCChainLink(ChainId.TESTNET, 'abc', 'address')).toEqual('https://testnet.snowtrace.io/address/abc')
    })
  })
