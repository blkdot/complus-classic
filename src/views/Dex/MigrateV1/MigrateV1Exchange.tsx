...
import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WAVAX } from '@complus/sdk-ava'
import React, { useCallback, useMemo, useState } from 'react'
import ReactGA from 'react-ga'
import { Redirect, RouteComponentProps } from 'react-router'
import { Text } from 'rebass'
import { ButtonConfirmed } from '../../../components/Dex/Button'
import { LightCard, PinkCard, YellowCard } from '../../../components/Dex/Card'
import { AutoColumn } from '../../../components/Dex/Column'
import CurrencyLogo from '../../../components/Dex/CurrencyLogo'
import QuestionHelper from '../../../components/Dex/QuestionHelper'
import { AutoRow, RowBetween, RowFixed } from '../../../components/Dex/Row'
import { Dots } from '../../../components/Dex/swap/styleds'
...
import { BackArrow, ExternalLink, TYPE } from '../../../theme/dexTheme'
import { getCChainLink, isAddress } from '../../../utils/dexUtils'
    ...
    <>
      ...
      <RowBetween my="1rem">
        <Text fontSize={16} fontWeight={500}>
          Pooled {chainId && token.equals(WAVAX[chainId]) ? 'WAVAX' : token.symbol}:
        </Text>
        <RowFixed>
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  ...
  const [v2PairState, v2Pair] = usePair(chainId ? WAVAX[chainId] : undefined, token)
  const isFirstLiquidityProvider: boolean = v2PairState === PairState.NOT_EXISTS

  const v2SpotPrice = chainId && v2Pair ? v2Pair.reserveOf(token).divide(v2Pair.reserveOf(WAVAX[chainId])) : undefined
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        ...
        {chainId && (
          <ExternalLink href={getCChainLink(chainId, MIGRATOR_ADDRESS, 'address')}>
            <TYPE.blue display="inline">Complus migration contract↗</TYPE.blue>
          </ExternalLink>
        )}
        .
      </TYPE.body>
      ...
      {!isFirstLiquidityProvider && largePriceDifference ? (
        <YellowCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            It{"'"}s best to deposit liquidity into Complus V2 at a price you believe is correct. If the V2 price seems
          ...
          <AutoColumn gap="8px">
            <RowBetween>
              ...
              <TYPE.black>
                {v1SpotPrice?.toSignificant(6)} {token.symbol}/AVAX
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v1SpotPrice?.invert()?.toSignificant(6)} AVAX/{token.symbol}
              </TYPE.black>
            </RowBetween>
            ...
            <RowBetween>
              ...
              <TYPE.black>
                {v2SpotPrice?.toSignificant(6)} {token.symbol}/AVAX
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v2SpotPrice?.invert()?.toSignificant(6)} AVAX/{token.symbol}
              </TYPE.black>
            </RowBetween>
      ...
      {isFirstLiquidityProvider && (
        <PinkCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            You are the first liquidity provider for this pair on Complus V2. Your liquidity will be migrated at the
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {`Your Complus V1 ${token.symbol}/AVAX liquidity will become Complus V2 ${token.symbol}/AVAX liquidity.`}
      </TYPE.darkGray>
    </AutoColumn>
  )
}
  ...
  const liquidityToken: Token | undefined = useMemo(
    () =>
      validatedAddress && chainId && token
        ? new Token(chainId, validatedAddress, 18, `UNI-V1-${token.symbol}`, 'Complus V1')
        : undefined,
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          <div>
            <QuestionHelper text="Migrate your liquidity tokens from Complus V1 to Complus V2." />
          </div>
        </AutoRow>

        {!account ? (
          <TYPE.largeHeader>You must connect an account.</TYPE.largeHeader>
        ) : validatedAddress && chainId && token?.equals(WAVAX[chainId]) ? (
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              Because Complus V2 uses WAVAX under the hood, your Complus V1 WAVAX/AVAX liquidity cannot be migrated. You
